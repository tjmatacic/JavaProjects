...
//			System.out.println(mid+":"+tbat_current_line);
	...
	public static long mergeAppendixToTBAT_OBTree (OBTree obtree, RandomAccessFile reader, 
		...
		ArrayList<Entry<Integer,Integer>> entry_list=obtree.getLeafEntryList();
		DiskAccessCount countTotal=new DiskAccessCount();
        int low=0;//low searching position when doing binary search
        for (Entry<Integer,Integer>entry: entry_list	){
        		int line_num_update=entry.getValue();
        		...
        		TBUN<Integer> tbun_updating=new TBUN<Integer>(timestamp.longValue(),tbat_oid.intValue(),value);
