...
public class DataCreator {
	...
	public static void prepareTBAT(int num_lines, String tbat_file_name) throws IOException{
		PrintWriter tbat_file = new PrintWriter(new FileWriter(tbat_file_name));
		String tbat_str="";
		String timestampstr="";
		timestampstr=String.format("%d", System.currentTimeMillis());
	...
	String update_file_name, int oid_position) throws IOException {
		...
		String format=oid_position==1?tbat_format:bat_format;
		...
		if(oid_position==0){
			...
			while(total_updated < update_num_lines){
				...
				timestampstr=timestampstr.substring(timestampstr.length()-8,timestampstr.length());
				current_line=rand.nextInt(num_lines)+1;
				update_file.format(tbat_format, timestampstr, current_line, -1);
				total_updated++;
			}
		}
		update_file.close();
	...
	/**
	 * version 4 creates tbat_formate update list 
	...
	String update_file_name) throws IOException {
		...
		while(total_updated < update_num_lines){
			...
			timestampstr=timestampstr.substring(timestampstr.length()-8,timestampstr.length());
			current_line=rand.nextInt(num_lines)+1;
			update_file.format(tbat_format, timestampstr, current_line, update_value++);
			total_updated++;
		}
		update_file.close();
	}
